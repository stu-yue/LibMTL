...
def check_GT(dir_path="../../gaittracker-1.0.0/"):
    ...
    tra_val_data, tes_data, tra_val_label, tes_label = train_test_split(
        all_data, all_label, test_size=0.2, random_state=90)
    tra_data, val_data, tra_label, val_label = train_test_split(
        tra_val_data, tra_val_label, test_size=0.25, random_state=90)
    
    np.save(os.path.join(dir_path, "train_data.npy"), tra_data)
    np.save(os.path.join(dir_path, "train_label.npy"), tra_label)
    np.save(os.path.join(dir_path, "val_data.npy"), val_data)
    np.save(os.path.join(dir_path, "val_label.npy"), val_label)
...
class GaitTackerDataset(Dataset):
    ...
    SLOPE_LABELS = [0, 5, 10]
    SPEED_LABELS = [2, 3, 4]
    TASK_INDEX = {"load": 1, "slope": 2, "speed": 3}
    """ data  shape : (num_samples, 2, seq, 12) 
        label shape : (num_samples, 4) """
    def  __init__(
        self,
        data_root="../../gaittracker-1.0.0/",
        mode="train",
        tasks=TASK_INDEX.keys(),
    ):
        super().__init__()
        ...
        print(f"data shape: {self.data.shape}, label shape: {self.label.shape}, tasks: {self.tasks}")
        self.c = self.data.shape[3]

    def _load_data(self,):
        assert self.mode in ['train', 'val', 'test'], "Dataset mode option: train, val, test"
        data_path = os.path.join(self.data_root, "{}_data.npy".format(self.mode))
        label_path = os.path.join(self.data_root, "{}_label.npy".format(self.mode))
    ...
    def __getitem__(self, index):
        ...
        return data, {task: label[self.TASK_INDEX[task]] for task in self.tasks}
...
if __name__ == "__main__":
    # check_GT()
    dataset = GaitTackerDataset(mode="train")
    dataset = GaitTackerDataset(mode="val")
    dataset = GaitTackerDataset(mode="test")